...
test('deployment copies and clamps reserve options and an ordinary restart clears deathmatch', () => {
    ...
    assert.deepEqual(snapshot(scene.battleOptions), { deathmatch: true, reserves: { red: 3, blue: 0 } });
    ...
    assert.deepEqual(snapshot(scene.battleOptions), { deathmatch: false, reserves: { red: 0, blue: 0 } });
